...
namespace Math
{
	...
	public:
		...
		//friend bool operator ==(const Mat<T, ROWS, COLS> &lhs, const Mat<T, ROWS, COLS> &rhs);
		//friend bool operator !=(const Mat<T, ROWS, COLS> &lhs, const Mat<T, ROWS, COLS> &rhs);
		
		////Arithmetic (compiler doesn't detect link to definition)
		//template<typename T, size_t N, size_t M, size_t O, size_t P>
		//friend void operator *(const Mat<T, N, M> &lhs, const Mat<T, O, P> &rhs);
		//template<typename T, size_t N, size_t M, size_t O>
		//friend Mat<T, N, O> operator *(const Mat<T, N, M> &lhs, const Mat<T, M, O> &rhs);
